...
Read each file and check against OWASP Top 10 categories. Use the `praxis-owasp-top-10` skill for detailed patterns per category.
...
Use the `praxis-security-audit-checklist` skill to verify comprehensive coverage. Focus on sections relevant to the changed files:
...
{Filled checklist items from praxis-security-audit-checklist for reviewed areas}
...
Use this role as a reference file for a Codex skill. If subagents are available and explicitly requested by the active skill, spawn a focused subagent with this role's scope and collect a concise artifact summary. If subagents are unavailable, apply the role inline and write the same artifact contract to disk.
