...
**Question categories** (use praxis-task-refinement skill for templates):
...
1. **Choose story format** based on praxis-task-refinement skill Format Selection Guide:
...
6. **Flag risks** using praxis-task-refinement skill Risk Flag Patterns
...
{Обраний формат зі skill praxis-task-refinement}
