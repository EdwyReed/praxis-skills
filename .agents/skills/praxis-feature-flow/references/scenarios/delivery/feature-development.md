...
skills:
  ...
  - praxis-design-template (Phase 2)
  - praxis-adr-template (Phase 2)
  - praxis-api-contracts-template (Phase 2)
  - praxis-stoplight-docs (Phase 5, if applicable)
...
Before any project-scoped phase:

1. Check `.praxis/project.md`; invoke `praxis-init` when missing.
2. Read the profile fully and validate it.
3. Require `status: confirmed` before Design, Plan, Implement, documentation writes, or PR delivery. Permit only initialization, read-only reconnaissance, and diagnostics while unconfirmed.
4. Store the validator-provided SHA-256 as `project_context.profile_sha256` in `state.json`.
5. When `.praxis/skills.yaml` exists, validate it and store `project_context.skills_manifest_sha256`; load its package details only for skill selection, availability, or installation, and never install without explicit approval.
5. Re-read after compaction, a hash change, or a request that changes product or visual direction.

The managed root `AGENTS.md` bootstrap makes this requirement discoverable even when Praxis Skills is not installed.

---

## Frontend Skill Gate

Before Phase 2, Phase 3, or Phase 4 begins for visually significant frontend work:
