...
/**
 * Full guided install wizard. Returns { scope, repoPath?, selectedAgents, confirmed }.
 */
export async function runInstallWizard(distribution, detections, options = {}) {
  ...
  note("Install plan", [
    `${c.bold}${selectedAgents.length}${c.reset} agent${selectedAgents.length === 1 ? "" : "s"}  ·  ${c.bold}${distribution.skills.length}${c.reset} skills  ·  scope ${c.bold}${scope}${c.reset}`,
    ...planLines,
  ], { stdout });
...
export function printInstallExperience(result, { stdout = process.stdout, version } = {}) {
  ...
  if (result.dryRun) {
    ...
    outro("Praxis is ready.", {
      stdout,
      hint: "Restart agent sessions so skill catalogs refresh. Then try $praxis-init or /praxis-init in Claude Code.",
    });
  }
  return true;
}
